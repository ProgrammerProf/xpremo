...
import { set_session, print } from "@/public/script/public";

export default function Card ({ data, big }) {

    let sale = Math.round(((data.old_price - data.new_price) / data.old_price) * 100);
    ...
    return (
        ...
        <Fragment>
            ...
            <div className="group text-center overflow-hidden w-full p-3 border mt-8 relative bg-white shadow-[0_3px_8px_0_rgba(0,3,143,0.1)]">

                <div className="icons flex z-20 flex-col gap-2 absolute -right-16 group-hover:right-4 top-2/4 -translate-y-1/2">

                    <button onClick={add_to_favore} className="group/edit hover:bg-[--main-color] rounded-full relative w-12 h-12 bg-slate-200">
                    ...
                    <button onClick={add_to_cart} className="group/edit hover:bg-[--main-color] rounded-full relative w-12 h-12 bg-slate-200">
...

                {
                    ...
                    <div>
                        ...
                        { data.old_price > 0 && <span className="z-10 absolute top-4 right-4 leading-7 bg-[#D01418] h-7 w-12 l text-sm  text-white font-bold">{sale}%</span> }

                        <div className="flex items-center">

                            <div className="w-[45%] relative">
                            
                                <img src={data.image} className="w-full object-cover group-hover:scale-50 group-hover:opacity-0 opacity-100 transition-all duration-200" width={300} height={150}/>
                            
                                <img src={data.image_hover} className="opacity-0 top-0 group-hover:opacity-100 w-full h-full object-cover group-hover:scale-100 transition-all duration-200 scale-50 absolute" width={300} height={150}/>
                            
                            </div>

                            <div className="text-right w-1/2 max-[640px]:text-sm">

                                <h1 className="text-[#0066C0] font-bold hover:underline capitalize">{data.name}</h1>

                                <Stars rate={data.rate}/>

                                <div className="price flex gap-3 mb-5 items-center max-[640px]:gap-2">

                                    <span className="font-bold text-xl max-[640px]:text-sm">{data.new_price} جنيه</span>

                                    { data.old_price > 0 && <span className="line-through text-gray-500 font-bold max-[640px]:text-sm">{data.old_price} جنيه</span> }

                                </div>

                                <p className="text-sm text-gray-640 max-[640px]:hidden max-h-[40px] text-[12px] over-hide w-[90%]">{data.description}</p>
                    ...
                    <div>
                        ...
                        <div className="w-[150px] m-auto mb-7 relative h-[150px] max-[640px]:w-[120px] max-[640px]:h-[105px]">

                            <img src={data.image} className="w-full duration-200 transition-all group-hover:scale-50 absolute group-hover:opacity-0 opacity-100" width={500} height={1000}/>
                            
                            <img src={data.image_hover} className="opacity-0 duration-200 transition-all group-hover:opacity-100 w-full h-full object-cover group-hover:scale-100 scale-50 absolute" width={200} height={500}/>

                        </div>

                        <h1 title={data.name} className="text-[#0066C0] font-bold text-sm mb-3 hover:underline capitalize max-[640px]:mb-1  whitespace-nowrap overflow-hidden text-ellipsis max-w-full">
                            {data.name}
                        </h1>

                        <div className="flex items-center justify-center"><Stars rate={data.rate}/></div>

                        <div className="flex flex-col h-[60px] justify-center">

                            <span className="font-medium text-lg max-[640px]:text-base">{data.new_price} جنيه</span>

                            { data.old_price > 0 && <span className="text-sm font-medium line-through text-gray-500">{data.old_price} جنيه</span> }
