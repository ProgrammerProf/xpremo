...
export default function ProductMiniCard ({ item }) {
    ...
    let sale2 = ((item.oldPrice - item.price) / item.oldPrice) * 100;
    let sale = Math.round(sale2);

    return (

        <div className="pb-8 group text-center overflow-hidden w-full p-3 border my-10 relative bg-white shadow-[0_3px_8px_0_rgba(0,3,143,0.1)]">
            ...
            <Link href={`product/${item.id}`}>
                ...
                { item.oldPrice && <span className="z-10 absolute top-4 right-4 leading-7 bg-[#D01418] h-7 w-12 l text-sm  text-white font-bold">{sale}%</span> }   

                <div className="w-[150px] m-auto mb-7 relative h-[150px] max-[640px]:w-[120px] max-[640px]:h-[105px]">
                   
                    <img src={item.img} className="w-full duration-200 transition-all group-hover:scale-50 absolute group-hover:opacity-0 opacity-100" width={500} height={1000} alt="_"/>
                    
                    <img src={item.imgHover} className="opacity-0 duration-200 transition-all group-hover:opacity-100 w-full h-full object-cover group-hover:scale-100 scale-50 absolute" width={200} height={500} alt="_"/>
                
                </div>

                <h1 title={item.name} className="text-[#0066C0] font-bold text-sm mb-3 hover:underline capitalize max-[640px]:mb-1  whitespace-nowrap overflow-hidden text-ellipsis max-w-full">
                    {item.name}
                </h1>

                <div className="flex items-center justify-center"><Stars rate={item.rate}/></div>

                <div className="flex flex-col gap-1 justify-center">

                    <span className="font-medium text-lg max-[640px]:text-base">{item.price} جنيه</span>

                    { item.oldPrice && <span className="absolute text-sm font-medium left-1/2 -translate-x-1/2 bottom-2 line-through text-gray-500 ">{item.oldPrice} جنيه</span> }
